  ...
  if (rxmb == 0)
    {
      ...
      work_queue(HPWORK, &priv->irqwork,
  ...
  if ((regval & CAN_TSR_RQCP0) != 0)
    {
      ...
      work_queue(HPWORK, &priv->irqwork, stm32can_txdone_work, priv, 0);
    }
...
static int stm32can_bittiming(struct stm32_can_s *priv)
{
  ...
  float tmp_brp = (float)STM32_PCLK1_FREQUENCY / (float)(priv->baud * CAN_BIT_QUANTA);
  brp = round(tmp_brp);
  ts1 = ceil((float)((CAN_BIT_QUANTA - 1) * 0.75));
  ts2 = floor((float)((CAN_BIT_QUANTA - 1) * 0.25));

  printf("TS1: %" PRIu32 " TS2: %" PRIu32 " BRP: %" PRIu32 " tmp %f\n",
               ts1, ts2, brp, tmp_brp);
  ...
  tmp = ((brp - 1) << CAN_BTR_BRP_SHIFT) | ((ts1 - 1) << CAN_BTR_TS1_SHIFT) |
        ((ts2 - 1) << CAN_BTR_TS2_SHIFT) | ((1) << CAN_BTR_SJW_SHIFT);
...
static int  stm32can_setup(struct stm32_can_s *priv)
{
  ...
  ninfo("CAN%" PRIu8 " RX0 irq: %" PRIu8 " RX1 irq: %" PRIu8
        ...
        priv->cansce);
#else
  printf("CAN%" PRIu8 " RX0 irq: %" PRIu8 " RX1 irq: %" PRIu8
        " TX irq: %" PRIu8 "\n",
        priv->port, priv->canrx[0], priv->canrx[1], priv->cantx);
#endif
  ...
  if (ret < 0)
    {
      printf("ERROR: Failed to attach CAN%" PRIu8 " RX0 IRQ (%" PRIu8 ")",
  ...
  if (ret < 0)
    {
      printf("ERROR: Failed to attach CAN%" PRIu8 " RX1 IRQ (%" PRIu8 ")",
...
void arm_netcaninitialize(void)
{
#ifdef CONFIG_STM32F7_CAN1
